	...
	public void tick() {
		...
		for(int i = 0; i < vetorRetangulos.size(); i++) {
			...
			if(Game.mouseClicado) {
				if(Game.mouse_y >= current.y && Game.mouse_x < current.x + current.width) {
					if(Game.mouse_y >= current.y && Game.mouse_y < current.y + current.height) {
						vetorRetangulos.remove(current);
						Game.pontuacao++;
						Game.mouseClicado = false;
						for(int n = 0; n < 50; n++) {
							objParticula = new Particula(current.x, current.y, 0, 0, current.cor);
